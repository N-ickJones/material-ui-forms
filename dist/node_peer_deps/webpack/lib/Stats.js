...
class Stats {
    ...
    toJson(options, forToString) {
        ...
        const requestShortener = compilation.compiler.context === context
            ...
            if (showReasons) {
                obj.reasons = module.reasons
                    ...
                    if (a.dependency && b.dependency) {
                        ...
                        if (cmp)
                            return cmp;
                        if (a.dependency.type < b.dependency.type)
                            return -1;
                        if (a.dependency.type > b.dependency.type)
                            return 1;
                    }
            ...
            for (const [origin, logEntries] of compilation.logging) {
                ...
                if (!debugMode) {
                    processedLogEntries = processedLogEntries.filter(entry => {
                        if (!acceptedTypes.has(entry.type))
                            return false;
                        if (!collapsedGroups) {
                            switch (entry.type) {
                ...
                processedLogEntries = processedLogEntries.map(entry => {
                    ...
                    return {
                        type: (debugMode || collapsedGroups) &&
