    ...
    apply(mainTemplate) {
        ...
        mainTemplate.hooks.localVars.tap("JsonpMainTemplatePlugin", (source, chunk, hash) => {
            ...
            if (needChunkOnDemandLoadingCode(chunk)) {
                extraCode.push("", "// script path function", "function jsonpScriptSrc(chunkId) {", Template.indent([
                    `return ${mainTemplate.requireFn}.p + ${getScriptSrcPath(hash, chunk, "chunkId")}`
                ]), "}");
            }
            ...
            return Template.asString([
                ...
                crossOriginLoading
                    ? Template.asString([
                        ...
                        Template.indent(`script.crossOrigin = ${JSON.stringify(crossOriginLoading)};`),
                        "}"
                    ])
                    : "",
        ...
        mainTemplate.hooks.linkPreload.tap("JsonpMainTemplatePlugin", (_, chunk, hash) => {
            ...
            return Template.asString([
                ...
                "link.charset = 'utf-8';",
                `if (${mainTemplate.requireFn}.nc) {`,
                Template.indent(`link.setAttribute("nonce", ${mainTemplate.requireFn}.nc);`),
                "}",
                'link.rel = "preload";',
                'link.as = "script";',
        ...
        mainTemplate.hooks.afterStartup.tap("JsonpMainTemplatePlugin", (source, chunk, hash) => {
            ...
            if (needChunkLoadingCode(chunk) &&
                ...
                return Template.asString([
                    source,
                    `webpackJsonpCallback([[], {}, 0, ${JSON.stringify(prefetchChunks)}]);`
                ]);
            }
            return source;
        });
        mainTemplate.hooks.startup.tap("JsonpMainTemplatePlugin", (source, chunk, hash) => {
            if (needEntryDeferringCode(chunk)) {
                if (chunk.hasEntryModule()) {
                    const entries = [chunk.entryModule].filter(Boolean).map(m => [m.id].concat(Array.from(chunk.groupsIterable)[0]
...
//# sourceMappingURL=JsonpMainTemplatePlugin.js.map