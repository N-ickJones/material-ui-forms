...
class ModuleConcatenationPlugin {
    ...
    apply(compiler) {
        compiler.hooks.compilation.tap("ModuleConcatenationPlugin", (compilation, { normalModuleFactory }) => {
            ...
            const setBailoutReason = (module, reason) => {
                ...
                module.optimizationBailout.push(typeof reason === "function"
            ...
            compilation.hooks.optimizeChunkModules.tap("ModuleConcatenationPlugin", (allChunks, modules) => {
                ...
                for (const module of modules) {
                    ...
                    if (!module.buildMeta ||
                        ...
                        !module.dependencies.some(d => d instanceof HarmonyCompatibilityDependency)) {
                    ...
                    if (module.variables.length > 0) {
                        setBailoutReason(module, `Module uses injected variables (${module.variables
                            .map(v => v.name)
                            .join(", ")})`);
                        continue;
                    }
                    // Hot Module Replacement need it's own module to work correctly
                    if (module.dependencies.some(dep => dep instanceof ModuleHotAcceptDependency ||
                    ...
                    const nonHarmonyReasons = module.reasons.filter(reason => !reason.dependency ||
                    ...
                    if (nonHarmonyReasons.length > 0) {
                        ...
                        const importingModuleTypes = new Map(Array.from(importingModules).map(m => /** @type {[string, Set]} */ ([
                            m,
                            new Set(nonHarmonyReasons
                                .filter(r => r.module === m)
                ...
                for (const concatConfiguration of concatConfigurations) {
                    ...
                    for (const reason of newModule.reasons) {
                        ...
                        if (reason.dependency.redirectedModule ===
    ...
    _tryToAdd(compilation, config, module, possibleModules, failureCache) {
        ...
        for (const reason of module.reasons) {
            ...
            if (reason.module.factoryMeta.sideEffectFree &&
