...
const compareEntries = (a, b) => {
    ...
    while (true) {
        ...
        if (aModuleIdentifier > bModuleIdentifier)
            return -1;
        if (aModuleIdentifier < bModuleIdentifier)
            return 1;
    }
...
module.exports = class SplitChunksPlugin {
    ...
    static normalizeName({ name, automaticNameDelimiter, automaticNamePrefix, automaticNameMaxLength }) {
        if (name === true) {
            ...
            const fn = (module, chunks, cacheGroup) => {
                ...
                names.sort();
                const prefix = typeof automaticNamePrefix === "string"
                    ? automaticNamePrefix
                    : cacheGroup;
                ...
                if (name.length > automaticNameMaxLength) {
                    ...
                    const sliceLength = automaticNameMaxLength -
    ...
    static normalizeFallbackCacheGroup({ minSize = undefined, maxSize = undefined, automaticNameDelimiter = undefined }, { minSize: defaultMinSize = undefined, maxSize: defaultMaxSize = undefined, automaticNameDelimiter: defaultAutomaticNameDelimiter = undefined }) {
        return {
                    ...
                    if (typeof option === "function") {
                        ...
                        results.push({
                            ...
                            getName: SplitChunksPlugin.normalizeName({
                                ...
                                automaticNameDelimiter: typeof option.automaticNameDelimiter === "string"
            ...
            compilation.hooks.optimizeChunksAdvanced.tap("SplitChunksPlugin", chunks => {
                ...
                for (const module of compilation.modules) {
                    ...
                    for (const cacheGroupSource of cacheGroups) {
                        const minSize = cacheGroupSource.minSize !== undefined
                            ? cacheGroupSource.minSize
                            : cacheGroupSource.enforce
                                ? 0
                                : this.options.minSize;
                        const enforceSizeThreshold = cacheGroupSource.enforceSizeThreshold !== undefined
                        ...
                        const cacheGroup = {
                            ...
                            minSize,
                            minSizeForMaxSize: cacheGroupSource.minSize !== undefined
                                ? cacheGroupSource.minSize
                                : this.options.minSize,
                            enforceSizeThreshold,
                            maxSize: cacheGroupSource.maxSize !== undefined
                        ...
                        for (const chunkCombination of combs) {
                            // Break if minimum number of chunks is not reached
                            if (chunkCombination.size < cacheGroup.minChunks)
                                continue;
                ...
                while (chunksInfoMap.size > 0) {
                    ...
                    if (item.cacheGroup.reuseExistingChunk) {
                        outer: for (const chunk of item.chunks) {
                            if (chunk.getNumberOfModules() !== item.modules.size)
                                continue;
                    ...
                    if (item.cacheGroup.filename) {
                        if (!newChunk.isOnlyInitial()) {
                            throw new Error("SplitChunksPlugin: You are trying to set a filename for a chunk which is (also) loaded on demand. " +
                    ...
                    if (!isReused) {
                        ...
                        for (const module of item.modules) {
                            if (typeof module.chunkCondition === "function") {
                                if (!module.chunkCondition(newChunk))
                                    continue;
                            }
                ...
                for (const chunk of compilation.chunks.slice()) {
                    const { minSize, maxSize, automaticNameDelimiter, keys } = maxSizeQueueMap.get(chunk) || this.options.fallbackCacheGroup;
                    if (!maxSize)
                        continue;
                    if (minSize > maxSize) {
                    ...
                    const results = deterministicGroupingForModules({
                        ...
                        getKey(module) {
                            ...
                            const name = module.nameForCondition
                                ? contextify(compilation.options.context, module.nameForCondition())
                                : ident.replace(/^.*!|\?[^?!]*$/g, "");
                    ...
                    for (let i = 0; i < results.length; i++) {
                        ...
                        if (i !== results.length - 1) {
                            ...
                            for (const module of group.items) {
                                if (typeof module.chunkCondition === "function") {
                                    if (!module.chunkCondition(newPart))
                                        continue;
                                }
                                // Add module to new chunk
