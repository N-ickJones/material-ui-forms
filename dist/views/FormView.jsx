...
export function FormView(props) {
    ...
    if (loadLocal) {
        return (<AlertDialog id="loadingLocal" title="Would you like to load your previous session?" description="Selecting Cancel will remove your previous session." color="primary" backLabel="Cancel" forwardLabel="Continue" onSubmit={handleLocalLoad} backOnOutsideClick={false}/>);
    }
    else if (loadServer) {
        ...
        <Snackbar open={alert.open} autoHideDuration={alert.autoHideDuration} onClose={() => { setAlert(Object.assign(Object.assign({}, alert), { open: false })); }} anchorOrigin={alert.origin}>
        ...
        <Grid className="d-flex flex-wrap">
          <AlertDialogButton id="refreshForm" className="m-1" label={locked ? <Lock color="primary"/> : <LockOpen color="primary"/>} title={locked ? "Unlock this form" : "Lock this form"} description={locked ? "Unlocking this form will allow you to make changes." : "Lock this form to prevent unwanted changes."} color="primary" backLabel="Cancel" forwardLabel={locked ? "Unlock" : "Lock"} onSubmit={handleLock}/>
          <AlertDialogButton id="saveForm" className={!locked ? "m-1 ml-auto" : "d-none"} label="Save" title="You have selected to save your current process locally." description="Warning: If you have local storage disabled then your changes 
        ...
        <Paper className={"p-3 print-paper"} ref={printComponentRef}>
          ...
          <Grid container spacing={3} className="print-container">
            ...
            return (<props.FormElement key={key} index={index} onChange={onChange} values={item} printMode={printMode} locked={locked} handleDelete={handleDelete} onChangeList={onChangeList} handleAddList={handleAddList} handleDeleteList={handleDeleteList}/>);
        })}
            ...
            <Grid item xs={12}>
              {props.forms.length < props.maxNodes && (<Grid className="d-flex">
                  <Button className={!locked ? "ml-auto" : "d-none"} variant="contained" color="primary" onClick={props.handleAddNewItem}>Add New Item</Button>
                </Grid>)}
              <Divider className="my-3"/>
            </Grid>
            ...
            <Grid item xs={12}>
                <Grid className="d-flex">
                  <AlertDialogButton id="saveForm" className="mx-auto d-flex w-50 mb-3" btnClass="w-100" label="Submit" title="You have selected to submit your information." description="This action will save this information to your user account." color="primary" backLabel="Cancel" forwardLabel="Submit" onSubmit={handleSubmit} disabled={props.handleSubmit === undefined}/>
                </Grid>
              </Grid>}
